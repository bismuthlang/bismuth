...
loop:                                             ; preds = %match-cont4, %entry
  br i1 true, label %ai-cond, label %rest

ai-cond:                                          ; preds = %loop
  %6 = load ptr, ptr %setRq, align 8
  %7 = call i1 @_ShouldLinearAcceptWhileLoop(ptr %6)
  br i1 %7, label %ai-then, label %rest

ai-then:                                          ; preds = %ai-cond
  %8 = load ptr, ptr %db, align 8
  %9 = call %Unit @_ContractChannel(ptr %8)
  %10 = load ptr, ptr %db, align 8
  %11 = call %Unit @_WriteProjection(ptr %10, i32 2)
  %12 = load ptr, ptr %setRq, align 8
...
rest:                                             ; preds = %ai-then, %ai-cond, %loop
  ...
  switch i32 %29, label %match-cont4 [
    i32 1, label %tagBranch1
    i32 2, label %tagBranch2
    i32 3, label %tagBranch3
  ]
...
tagBranch1:                                       ; preds = %rest
  ...
  %a1 = load %"(Unit + Value)", ptr %a, align 4
  %43 = call ptr @malloc(i32 8)
  store %"(Unit + Value)" %a1, ptr %43, align 4
...
loop6:                                            ; preds = %loop6, %rest5
  ...
  br i1 %122, label %loop6, label %rest7
