...
std::optional<ErrorChain *> SemanticVisitor::provisionFwdDeclSymbols(BismuthParser::CompilationUnitContext *ctx)
{
    ...
    for (auto e : ctx->defs)
    {
        ...
        DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(
            DefinitionSymbol,
            defSym,
...
std::variant<std::vector<DefinitionSignature *>, ErrorChain *> SemanticVisitor::defineFwdDeclSymbols(BismuthParser::CompilationUnitContext *ctx)
{
    ...
    {
        DEBUG_CERR("Forward Decl Impl." + implCtx->getText());
...
std::optional<ErrorChain *> SemanticVisitor::postCUVisitChecks(BismuthParser::CompilationUnitContext *ctx)
{
    ...
    if (auto opt = stmgr.lookup("main"); opt.has_value())
    {
        Symbol& sym = opt.value().get();
        
        if (std::optional<const TypeProgram *> progOpt = type_cast<TypeProgram>(sym.getType());
            progOpt.has_value())
        {
...
std::variant<TInvocationNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::CallExprContext *ctx)
{
    ...
    for (unsigned int i = 0; i < ctx->inv_args()->args.size(); i++)
    {
        ...
        DEFINE_OR_PROPAGATE_VARIANT_WMSG(
            TypedNode *, 
            provided, 
            anyOpt2VarError<TypedNode>(errorHandler, ctx->inv_args()->args.at(i)->accept(this)), 
            ctx->inv_args()->args.at(i),
            "Unable to generate argument");
 ...
 */
std::variant<ParameterListNode, ErrorChain *>  SemanticVisitor::visitCtx(BismuthParser::ParameterListContext *ctx)
{
    ...
    for (auto param : ctx->params)
    {
        ...
        if (prevUse != map.end())
        {
            return errorHandler.addError(param->getStart(), "Re-use of previously defined parameter " + name);
        }
        
        map.insert({name, param});

        DEFINE_OR_PROPAGATE_VARIANT_WMSG(ParameterNode, pn, this->visitCtx(param), ctx, "Failed to generate case type");
...
std::variant<TExternNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ExternStatementContext *ctx)
{
    ...
    std::variant<ParameterListNode, ErrorChain *> tyOpt = (ctx->paramList) ? this->visitCtx(ctx->paramList)
                                                              : ParameterListNode();

    DEFINE_OR_PROPAGATE_VARIANT_WMSG(ParameterListNode, paramNode, tyOpt, ctx, "Failed to generate parameters for extern!");
    ...
    for (ParameterNode param : paramNode)
    {
    ...
    DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(
        ...
        ctx, 
        "Could not add definition for " + id
    );
...
std::variant<TVarDeclNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::VariableDeclarationContext *ctx)
{
  ...
  for (auto e : ctx->assignments)
  {
    ...
    for (auto var : e->VARIABLE())
    {
      ...
      auto symOpt = stmgr.lookupInCurrentScope(id);
      if (symOpt)
      {
      ...
      if (e->a)
      {
        DEFINE_OR_PROPAGATE_VARIANT_WMSG(TypedNode *, exprOptO, anyOpt2VarError<TypedNode>(errorHandler, e->a->accept(this)), ctx,  "Unable to type check assignment");
        exprOpt = exprOptO;
      }
...
std::variant<TMatchStatementNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::MatchStatementContext *ctx)
{
  ...
  auto branchOpt = checkBranch<BismuthParser::MatchStatementContext, BismuthParser::MatchAlternativeContext, std::pair<std::reference_wrapper<Symbol>, TypedNode *>>(
    ctx,
    ...
    false,
    [](std::pair<std::reference_wrapper<Symbol>, TypedNode *> pair) -> TypedNode * { return pair.second; },
    [this, ctx, sumType, &foundCaseTypes](BismuthParser::MatchAlternativeContext *altCtx) -> std::variant<std::pair<std::reference_wrapper<Symbol>, TypedNode *>, ErrorChain *>
    {
      ...
      DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, local, stmgr.addSymbol(altCtx->name->getText(), caseType, false), altCtx, "Duplicate case name"); // TODO: may provide duplicate errors to prior checks!
      ...
      return std::make_pair(std::ref(local), ans);
    });
  ...
  DEFINE_OR_PROPAGATE_VARIANT_WMSG(
    MACRO_ARG(ConditionalData<std::pair<std::reference_wrapper<Symbol>, TypedNode *>>),
    dat,
    branchOpt,
    ctx, 
    "Failed to type check match statement"
  );
...
std::variant<TWhileLoopNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ProgramLoopContext *ctx)
{
    ...
    return new TWhileLoopNode(checkNode, blk, ctx->getStart());
}
...
std::variant<TBlockNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ForStatementContext *ctx)
{
    ...
    exprs.push_back(new TWhileLoopNode(check, block, ctx->getStart()));
...
std::variant<TConditionalStatementNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ConditionalStatementContext *ctx)
{
    ...
    DEFINE_OR_PROPAGATE_VARIANT(TypedNode *, cond, this->visitCtx(ctx->check), ctx); 
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(ConditionalData<TBlockNode *>, dat, branchOpt, ctx, "Failed to generate one or more cases in if statement");

    if (ctx->falseBlk)
    {
        return new TConditionalStatementNode(ctx->getStart(), cond, dat.cases.at(0), dat.post, dat.cases.at(1));
    }

    return new TConditionalStatementNode(ctx->getStart(), cond, dat.cases.at(0), dat.post);
}
...
std::variant<TSelectStatementNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::SelectStatementContext *ctx)
{
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(ConditionalData<TSelectAlternativeNode *>, dat, branchOpt, ctx, "Failed to type check select statement");
...
std::variant<TReturnNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ReturnStatementContext *ctx)
{
     ...
     */
    DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol , sym, stmgr.lookup("@RETURN"), ctx, "Cannot use return outside of a function");
...
std::variant<TExitNode *, ErrorChain *> SemanticVisitor::visitCtx(BismuthParser::ExitStatementContext *ctx)
{
    ...
    if (auto symOpt = stmgr.lookup("@EXIT"); !symOpt.has_value())
    {
...
SemanticVisitor::visitCtx(BismuthParser::LambdaTypeContext *ctx)
{
    ...
    for (auto param : ctx->paramTypes)
    {
        ...
        DEFINE_OR_PROPAGATE_VARIANT_WMSG(const Type *, type, anyOpt2VarError<const Type>(errorHandler, param->accept(this)), ctx, "Failed to generate parameter type");
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(
        const Type *, 
        returnType, 
        anyOpt2VarError<const Type>(errorHandler, ctx->returnType->accept(this)), 
        ctx, 
        "Error generating return type"
    );
...
SemanticVisitor::visitPathType(BismuthParser::PathContext *ctx)
{
    if(ctx->eles.size() == 1)
    {
        ...
        DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, sym, stmgr.lookup(name), pCtx, "Undefined type: " + name); // TODO: address inefficiency in var decl where this is called multiple times
    ...
    for(auto pCtx : ctx->eles)
    {
        ...
        DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, sym, lookupScope.get().lookupInAccessableScopes(stepId), pCtx, "Could not find " + stepId + " in " + lookupScope.get().getIdentifier()->getFullyQualifiedName());
        // if (sym->getType()->isLinear())
        // {
        //     if (!is_rvalue)
        //     {
        ...
        auto defSym = symbol_cast<const DefinitionSymbol>(sym);
        if(!defSym)
        ...
        lookupScope = defSym.value().get().getInnerScope();
    }
...
SemanticVisitor::visitCtx(BismuthParser::ProgramTypeContext *ctx)
{
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(const ProtocolSequence *, proto, protoOpt, ctx, "Failed to generate protocol type for program");
...
std::variant<TChannelCaseStatementNode *, ErrorChain *> SemanticVisitor::TvisitProgramCase(BismuthParser::ProgramCaseContext *ctx)
{
    ...
    for (auto alt : ctx->protoAlternative())
    {
      ...
      DEFINE_OR_PROPAGATE_VARIANT_WMSG(const ProtocolSequence *, a, protoOpt, ctx, "Failed to generate protocol type in channel case statement");
    ...
    auto branchOpt = checkBranch<BismuthParser::ProgramCaseContext, BismuthParser::StatementContext, TypedNode *>(
      ...
      [this, fullSequences, &branch, id](BismuthParser::StatementContext *alt) -> std::variant<TypedNode *, ErrorChain *>
      {
        ...
        DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, sym, stmgr.lookup(id), alt, "Could find channel: " + id);
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(ConditionalData<TypedNode *>, dat, branchOpt, ctx, "Failed to type check external choice");
...
std::variant<TProgramProjectNode *, ErrorChain *> SemanticVisitor::TvisitProgramProject(BismuthParser::ProgramProjectContext *ctx)
{
    ...
    DEFINE_OR_PROPAGATE_VARIANT_WMSG(const ProtocolSequence *, ps, protoOpt, ctx, "Failed to generate protocol type in project statement");
...
std::variant<TProgramAcceptNode *, ErrorChain *> SemanticVisitor::TvisitProgramAccept(BismuthParser::ProgramAcceptContext *ctx)
{
    ...
    DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, sym, stmgr.lookup(id), ctx, "Unbound identifier: " + id);
    ...
    // If there are any uninferred symbols, then add it as an error as we won't be able to resolve them
    // due to the var leaving the scope
    if (auto lins = stmgr.getLinears(SymbolLookupFlags::PENDING_LINEAR);
        lins.size() > 0)
    {
      std::ostringstream details;
      for (Symbol& e : lins)
    ...
    DEFINE_OR_PROPAGATE_VARIANT(TBlockNode *, blk, blkOpt, ctx);

    return new TProgramAcceptNode(sym, isInCloseable, blk, ctx->getStart());
}
...
std::variant<TProgramAcceptWhileNode *, ErrorChain *> SemanticVisitor::TvisitProgramAcceptWhile(BismuthParser::ProgramAcceptWhileContext *ctx)
{
    ...
    return new TProgramAcceptWhileNode(sym, isInCloseable, condition, blk, ctx->getStart());
}
...
std::variant<TProgramAcceptIfNode *, ErrorChain *> SemanticVisitor::TvisitProgramAcceptIf(BismuthParser::ProgramAcceptIfContext *ctx)
{
    ...
    auto branchOpt = checkBranch<BismuthParser::ProgramAcceptIfContext, BismuthParser::BlockContext, TBlockNode *>(
      ...
      [this, &idx, id, proto_post_accept](BismuthParser::BlockContext *blk) -> std::variant<TBlockNode *, ErrorChain *>
      {
        ...
        {
          idx++;
          DEFINE_OR_PROPAGATE_OPTIONAL_REF_WMSG(Symbol, sym, stmgr.lookup(id), blk, "Could not find channel: " + id); // FIXME: FIND BETTER WAY TO MAP AND CHANGE CHANNEL VALUES IN SPECIFIC BRANCHES
    ...
    std::function<std::variant<Y, ErrorChain *>(T *)> typeCheck)
{
    ...
    const auto checkCase = [&](auto * branchToken, bool checkRest, std::string branchErrorMessage, std::string subsequentErrorMessage) -> std::optional<ErrorChain *>{
            ...
            lins.size() > 0)
        {
            ...
            return errorHandler.addError(branchToken->getStart(), "588 Unused linear types in context: " + details.str());
        }
...
/// NOTE: THIS RETURNS THE ORIG SCOPE
std::variant<std::reference_wrapper<Scope>, ErrorChain*> SemanticVisitor::enterTemplateScope(DefinitionSymbol& defSym, VisibilityModifier m, TemplateInfo info)
{
    Scope& origScope = stmgr.getCurrentScope();
    ...
    {
        optional_ref<DefinitionSymbol> symOpt =  stmgr.addDefinition(m, i.first, i.second, false);
...
SemanticVisitor::defineTemplateType(BismuthParser::DefineTypeContext *ctx, const TypeTemplate *templateTy, DefinitionSymbol& defSym, VisibilityModifier m)
{
    ...
    if(BismuthParser::DefineFunctionContext * fnCtx = dynamic_cast<BismuthParser::DefineFunctionContext *>(ctx))
    {
        ...
        DEFINE_OR_PROPAGATE_VARIANT_REF(
            Scope,
            origScope,
            enterTemplateScope(const_cast<DefinitionSymbol&>(defSym), m, info),
            fnCtx
        );
    ...
    if(BismuthParser::DefineProgramContext * progCtx = dynamic_cast<BismuthParser::DefineProgramContext *>(ctx)) {
        // TODO: get errors from this?
        TemplateInfo info = TvisitGenericTemplate(progCtx->genericTemplate());
    ...
    if(BismuthParser::DefineStructContext * structCtx = dynamic_cast<BismuthParser::DefineStructContext *>(ctx)) {
        // TODO: get errors from this?
        TemplateInfo info = TvisitGenericTemplate(structCtx->genericTemplate());

        TypeStruct * structTy = new TypeStruct(); //structCtx->name->getText());
    ...
    if(BismuthParser::DefineEnumContext * enumCtx = dynamic_cast<BismuthParser::DefineEnumContext *>(ctx)) {
        TemplateInfo info = TvisitGenericTemplate(enumCtx->genericTemplate());

        TypeSum * sumTy = new TypeSum();//enumCtx->name->getText());
    ...
    if(BismuthParser::DefineTraitContext * traitCtx = dynamic_cast<BismuthParser::DefineTraitContext *>(ctx)) {
        // TODO: get errors from this?
        TemplateInfo info = TvisitGenericTemplate(traitCtx->genericTemplate());

        TypeTrait * traitTy = new TypeTrait(); //traitCtx->name->getText());
